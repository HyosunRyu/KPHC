...
* 따라서 **정치적 혐오표현**은 정치적 반대편에 대한 부정적인 편견과 고정관념을 바탕으로 상대후보 또는 그 지지자들을 모욕·비하·멸시·위협하거나 그들에 대한 차별·폭력을 선동하는 행위로 정의되며, 그 중에서도 차별과 폭력 등 구체적인 행동을 선동하는 발언은 **정치적 증오선동**으로 구분됩니다.
...
* 신뢰도 평가를 위해 급내상관계수(Intraclass Correlation Coefficient, ICC)를 계산함.
...
* KcELECTRA는 한국어 온라인 댓글 3.4억 건을 사전 학습한 사전 학습 모델로, 댓글과 같은 이용자 생성 텍스트 처리에 특화된 모델입니다.
...
* [정치적 증오선동 분류 모델 학습](https://github.com/HyosunRyu/KPHC/blob/main/KPHC_(Incitement).ipynb)
...
## 6. 인용 (Citation)
KPHC를 인용할 때는 아래 양식을 이용해주세요.

```
@misc{Ryu2024KPHC,
   title={Korean Political Hate Speech Classifier},
   author={Hyo-sun Ryu and Jae Kook Lee},
   year={2024},
   publisher = {GitHub},
   journal = {GitHub repository},
   howpublished = {\url{https://github.com/HyosunRyu/KPHC}
}
```
